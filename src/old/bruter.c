...
void recursiveUnset(Table *state, char* key) 
{
    ...
    while (splited->size > 1) 
    {
        ...
        {
            Array *array = v.value.p;
            currentKey = ArrayShift(splited).value.s;
            MaxSize index = (MaxSize)atof(currentKey);
            if (index < array->size) 
            {
                Variable tmp = ArrayRemove(array, index);
...
Variable bulkInterpret(Table *state, char* input) 
{
...
Variable FreeTable(Table* state, Array* args) 
{
    Function FreeArray = (Function)TableGet(state, "array.free").value.p;
    Table* table = (Table*)ArrayShift(args).value.p;
    for (int i = 0; i < table->size; i++) 
    {
        Variable v = table->data[i];
        if (v.type == TYPE_TABLE) 
        {
            Array* args = createNewArray();
            ArrayPush(args, (Variable){.type = TYPE_TABLE, .value = {.p = v.value.p}});
            FreeTable(state, args);
            ArrayFree(args);
            free(args);
        }
        else if (v.type == TYPE_ARRAY) 
...
Variable FreeArray(Table* state, Array* args) 
{
    Function _freeTable = (Function)TableGet(state, "table.free").value.p;
    Function _freeArray = (Function)TableGet(state, "array.free").value.p;
    Array* array = (Array*)ArrayShift(args).value.p;
    for (int i = 0; i < array->size; i++) 
    {
        Variable v = array->data[i];
        if (v.type == TYPE_TABLE) 
        {
            Array* args = createNewArray();
            ArrayPush(args, (Variable){.type = TYPE_TABLE, .value = {.p = v.value.p}});
            _freeTable(state, args);
            ArrayFree(args);
            free(args);
        }
        else if (v.type == TYPE_ARRAY) 
...
int main(int argc, char** argv) 
{
    ...
    registerFunction(&state, "table.free", FreeTable);
    ...
    registerFunction(&state, "array.free", FreeArray);
